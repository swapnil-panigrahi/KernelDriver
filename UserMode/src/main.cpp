...
static std::uintptr_t get_module_base(const DWORD pid, const wchar_t* module_name) {
	...
	HANDLE snap_shot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS | TH32CS_SNAPMODULE32, pid);
	if (snap_shot == INVALID_HANDLE_VALUE)
		return module_base;
...
int main() {
	...
	const HANDLE driver = CreateFile(L"\\\\.\\KDriver", GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
